...
pub fn establish_connection() -> SqliteConnection {
  dotenvy::dotenv().ok();
  let default_db_buf = PathBuf::from("data").join("meme.db");
  let default_db_path = default_db_buf.to_str().unwrap();
  let database_url = env::var("DATABASE_URL").unwrap_or(String::from(default_db_path));
